...
sub isAdmin($){
	...
	my $con = DBI->connect( GetDB(), GetID(), GetPW() );
		my $state=$con->prepare("SELECT count(*) FROM admin WHERE ui_id=\'$id\'");
		$state->execute;
		my @row=$state->fetchrow_array;
		if($row[0]!=0){
			$admin=1;
		}else{
			$state=$con->prepare("SELECT count(*) FROM superadmin WHERE ui_id=\'$id\'");
			$state->execute;
...
<div class="message-box">
  ...
  <div class="buttons-con">
    <div class="action-link-wrap">
      ...
      <a href="../main.pl" class="link-button">Go to Home Page</a>
    </div>
  </div>
</div>
</body></html>';
